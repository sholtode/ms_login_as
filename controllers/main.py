...
class LoginAsHome(Home):
    ...
    def login_as(self, uid=None, **kwargs):
        ...
        if uid and user.has_group('ms_login_as.group_login_as'):
            if not request.httprequest.cookies.get(SCOPE):
                key = user.env['res.users.apikeys']._generate(SCOPE, uuid4().hex)
...
class LoginAsSession(Session):
    ...
    def _get_origin_user_id(self):
        ...
        if key and request.env:
            result = request.env['res.users.apikeys']._check_credentials(scope=SCOPE, key=key)
            if result:
